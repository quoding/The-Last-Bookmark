...
export const TIMES = ['20:30', '20:32', '20:34', '20:37', '20:39', '20:41', '20:45', '20:47', '20:50', '20:53', '20:55', '20:58', '21:00'];
export const SCENE_NAMES = ['마지막 손님', '남겨둔 책', '쓰지 못한 한 문장', '문을 닫기 전에'];
...
export const sceneForTurn = (turn: number) => Math.min(4, Math.floor(turn / 3) + 1);
export const initialTurn: TurnResponse = {
  messages: [
    { id: 'm_initial_n', turn: 0, kind: 'narration', text: SCENE_ENTRIES[0] },
    { id: 'm_initial_r', turn: 0, kind: 'reply', text: '오늘은 책 사러 오셨다고 해도 못 팔아요. 계산대를 먼저 꺼버렸거든요. 마지막 정리 중인데… 잠깐 있다 가실래요?' },
  ], completed_turns: 0, story_time: TIMES[0], scene: { id: 1, name: SCENE_NAMES[0], entered: true, image_url: '/images/scene-1.svg' }, card_available: false, is_final_turn: false,
};
export const mockTurns: TurnResponse[] = INPUTS.map((text, index) => {
  ...
  if (turn === 3) messages.push({ id: 'm_3_record', turn, kind: 'record', record_type: 'fact', text: '서윤은 다른 서점에서 주 삼 일 일하며 쉬어볼 생각이다.' });
  if (turn === 5) messages.push({ id: 'm_5_record', turn, kind: 'record', record_type: 'memory', text: '책갈피를 계속 쓰고 있다는 말이 서윤에게 남았다.' });
  if (turn === 9) messages.push({ id: 'm_9_record', turn, kind: 'record', record_type: 'promise', text: '책을 다 읽으면 감상을 보내기로 하고 연락처를 나눴다.' });
  // 전환 응답의 마지막 narration을 진입 서술로 쓰는 규약. 새 필드는 추가하지 않는다.
  if (entered) messages.push({ id: `m_${turn}_entry`, turn, kind: 'narration', text: SCENE_ENTRIES[scene - 1] });
  return { messages, completed_turns: turn, story_time: TIMES[turn], scene: { id: scene, name: SCENE_NAMES[scene - 1], entered, image_url: `/images/scene-${scene}.svg` }, card_available: scene === 3 && turn < 8, is_final_turn: turn === 12 };
});
export const mockEnding: Ending = {
  ...
  evidence: [
    { message_id: 'm_5_p', turn: 5, story_time: '20:41', scene_name: SCENE_NAMES[1], quote: INPUTS[4], effect: '계속 쓰고 있다는 이 말이, 책방 밖에서도 책 이야기를 이어갈 바탕이 되었다.' },
    { message_id: 'm_8_p', turn: 8, story_time: '20:50', scene_name: SCENE_NAMES[2], quote: INPUTS[7], effect: '이 문장이 카드에 남아, 아직 정해지지 않은 다음을 서두르지 않게 했다.' },
...
export const mockSessions: SessionList = { sessions: [
  ...
  { id: 'story-2', index: 2, status: 'completed', completed_turns: 12, ending_title: mockEnding.title, portrait_url: '/images/portrait-2.svg', created_at: '2026-09-06T20:11:00+09:00' },
  { id: 'story-1', index: 1, status: 'completed', completed_turns: 12, ending_title: mockEnding.title, portrait_url: '/images/portrait-1.svg', created_at: '2026-09-05T20:11:00+09:00' },
] };
