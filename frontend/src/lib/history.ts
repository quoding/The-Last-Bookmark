 ...
 * 과거 장면 이름과 이미지는 서버 scenes에서 복원하며 URL을 만들어내지 않는다.
 */
  ...
  return Array.from({ length: state.completed_turns + 1 }, (_, turn) => {
    ...
    return {
      ...
      scene: {
        ...
        name: current ? state.scene.name : (scene?.name ?? names[sceneId - 1]),
        entered: [0, 3, 6, 9].includes(turn),
        image_url: current ? state.scene.image_url : (scene?.image_url ?? null),
      },
