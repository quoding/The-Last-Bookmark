...
export const limitCard = (text: string) =>
  graphemes(text).slice(0, 80).join("");
...
export function splitTransition(turn: TurnResponse) {
  ...
  const entry =
    turn.scene.entered && turn.completed_turns > 0 && last?.kind === "narration"
      ? last
      : undefined;
  return {
    reaction: entry ? turn.messages.slice(0, -1) : turn.messages,
    entry,
  };
}
